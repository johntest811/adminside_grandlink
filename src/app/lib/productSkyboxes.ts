...
export function describeSkyboxSource(weather: WeatherKey, productSkyboxes?: SkyboxMap | null, sharedDefaults?: SkyboxMap | null): string | null {
  const product = normalizeSkyboxes(productSkyboxes);
  const shared = normalizeSkyboxes(sharedDefaults);

  if (product[weather]) return `${weather} custom skybox`;
  if (shared[weather]) return `${weather} shared default skybox`;
  if (product.default) return "product default skybox fallback";
  if (shared.default) return "shared default skybox fallback";
  return null;
}
